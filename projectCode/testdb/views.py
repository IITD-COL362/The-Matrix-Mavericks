...
def filter_restaurant(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			if (fcountry=='Select') :
				with connection.cursor() as cursor:
					cursor.execute(f"with var as (select * from restaurant where ({is_name} or name='{fname}') and ({is_city} or city_id='{fcity}') and ({is_htb} or has_table_booking='{fhtb}') and ({is_hod} or has_online_delivery='{fhod}') and ({is_idn} or is_delivering_now='{fidn}') and ({is_stom} or switch_to_order_menu='{fstom}') and ({is_rating} or aggregate_rating>={frating}) and ({is_votes} or votes>={fvotes}) order by aggregate_rating desc),nvar as (select * from var,city where var.city_id=city.city_id),fvar as (select * from (select * from nvar,country_currency where nvar.country_name=country_currency.country_name) as tvar,currency where tvar.currency_id=currency.currency_id) select * from fvar where ({is_avgcost} or (avg_cost_for_two*inr_conversion>={favgcost1} and avg_cost_for_two*inr_conversion<{favgcost2})) order by aggregate_rating desc")
					restaurants = cursor.fetchall()
			else :
				with connection.cursor() as cursor:
					cursor.execute(f"with var as (select * from restaurant where ({is_name} or name='{fname}') and ({is_city} or city_id='{fcity}') and ({is_htb} or has_table_booking='{fhtb}') and ({is_hod} or has_online_delivery='{fhod}') and ({is_idn} or is_delivering_now='{fidn}') and ({is_stom} or switch_to_order_menu='{fstom}') and ({is_rating} or aggregate_rating>={frating}) and ({is_votes} or votes>={fvotes}) order by aggregate_rating desc),nvar as (select * from var,city where var.city_id=city.city_id and country_name='{fcountry}'),fvar as (select * from (select * from nvar,country_currency where nvar.country_name=country_currency.country_name) as tvar,currency where tvar.currency_id=currency.currency_id) select * from fvar where ({is_avgcost} or (avg_cost_for_two*inr_conversion>={favgcost1} and avg_cost_for_two*inr_conversion<{favgcost2})) order by aggregate_rating desc")
					restaurants = cursor.fetchall()
...
def filter_mix(request) :
	...
	if request.method == 'POST':
		...
		if form.is_valid() :
			...
			if (fcountry=='Select') :
				with connection.cursor() as cursor:
					cursor.execute(f"with var as (select * from food where ({is_cuisine} or cuisine_id='{fcuisine}') and ({is_calorie} or (calories>={fcalorie1} and calories<={fcalorie2})) and ({is_fat} or (fat<={ffat2} and fat>={ffat1})) and ({is_carbohydrate} or (carbohydrates>={fcarbohydrate1} and carbohydrates<={fcarbohydrate2})) and ({is_protein} or (protein>={fprotein1} and protein<={fprotein2})) and ({is_sodium} or (sodium>={fsodium1} and sodium<={fsodium2})) and ({is_meal} or meal_type_id='{fmeal}') and ({is_veg} or veg_non_veg='{fveg}') order by food_id asc),nvar as (select cuisine_id,count(*) as noi from var group by cuisine_id),fvar as (select restaurant_id,sum(noi) as tnoi from restaurant_cuisine,nvar where restaurant_cuisine.cuisine_id=nvar.cuisine_id group by restaurant_id),ddvar as (select * from (select * from restaurant,fvar where restaurant.restaurant_id=fvar.restaurant_id) as tab where ({is_name} or name='{fname}') and ({is_city} or city_id='{fcity}') and ({is_htb} or has_table_booking='{fhtb}') and ({is_hod} or has_online_delivery='{fhod}') and ({is_idn} or is_delivering_now='{fidn}') and ({is_stom} or switch_to_order_menu='{fstom}') and ({is_rating} or aggregate_rating>={frating}) and ({is_votes} or votes>={fvotes}) order by aggregate_rating desc),ddnvar as (select * from ddvar,city where ddvar.city_id=city.city_id),ddfvar as (select * from (select * from ddnvar,country_currency where ddnvar.country_name=country_currency.country_name) as ddtvar,currency where ddtvar.currency_id=currency.currency_id) select * from ddfvar where ({is_avgcost} or (avg_cost_for_two*inr_conversion>={favgcost1} and avg_cost_for_two*inr_conversion<{favgcost2})) order by tnoi desc,name asc")
					restaurants = cursor.fetchall()
			else :
				with connection.cursor() as cursor:
					cursor.execute(f"with var as (select * from food where ({is_cuisine} or cuisine_id='{fcuisine}') and ({is_calorie} or (calories>={fcalorie1} and calories<={fcalorie2})) and ({is_fat} or (fat<={ffat2} and fat>={ffat1})) and ({is_carbohydrate} or (carbohydrates>={fcarbohydrate1} and carbohydrates<={fcarbohydrate2})) and ({is_protein} or (protein>={fprotein1} and protein<={fprotein2})) and ({is_sodium} or (sodium>={fsodium1} and sodium<={fsodium2})) and ({is_meal} or meal_type_id='{fmeal}') and ({is_veg} or veg_non_veg='{fveg}') order by food_id asc),nvar as (select cuisine_id,count(*) as noi from var group by cuisine_id),fvar as (select restaurant_id,sum(noi) as tnoi from restaurant_cuisine,nvar where restaurant_cuisine.cuisine_id=nvar.cuisine_id group by restaurant_id),ddvar as (select * from (select * from restaurant,fvar where restaurant.restaurant_id=fvar.restaurant_id) as tab where ({is_name} or name='{fname}') and ({is_city} or city_id='{fcity}') and ({is_htb} or has_table_booking='{fhtb}') and ({is_hod} or has_online_delivery='{fhod}') and ({is_idn} or is_delivering_now='{fidn}') and ({is_stom} or switch_to_order_menu='{fstom}') and ({is_rating} or aggregate_rating>={frating}) and ({is_votes} or votes>={fvotes}) order by aggregate_rating desc),ddnvar as (select * from ddvar,city where ddvar.city_id=city.city_id and country_name='{fcountry}'),ddfvar as (select * from (select * from ddnvar,country_currency where ddnvar.country_name=country_currency.country_name) as ddtvar,currency where ddtvar.currency_id=currency.currency_id) select * from ddfvar where ({is_avgcost} or (avg_cost_for_two*inr_conversion>={favgcost1} and avg_cost_for_two*inr_conversion<{favgcost2})) order by tnoi desc,name asc")
					restaurants = cursor.fetchall()
